  ...
  const fetchData = async () => {
    try {
      const [userRes, pupilRes] = await Promise.all([
        countUsersBySeason(startYear, endYear),
        countPupilsBySeason(startYear, endYear),
      ]);
      ...
      users.forEach((item) => {
        const [seasonKey, y] = item.label.split("-");
        const seasonLabel = getSeasonLabel(seasonKey);
        ...
        if (!map[seasonLabel]) map[seasonLabel] = {};
        if (!map[seasonLabel][y]) map[seasonLabel][y] = [];
        map[seasonLabel][y].push({ total: item.total, type });
      });
