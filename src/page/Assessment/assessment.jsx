...
const Assessment = () => {
    ...
    const [isDetailModalOpen, setIsDetailModalOpen] = useState(false); // State mới cho modal chi tiết
    const [selectedAssessment, setSelectedAssessment] = useState(null); // State lưu assessment được chọn để xem chi tiết
                        ...
                        <tbody>
                            {currentAssessments.map((assessment, index) => (
                                <tr key={assessment.id} className="border-t">
                                    <td className="p-3">{indexOfFirstAssessment + index + 1}</td>
